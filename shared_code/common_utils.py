...
class ZooplusSimulator:
    ...
    def __init__(self, n_products=50, n_users=1000, seed=42):
        ...
        # Define personas first to establish the single source of truth
        self.personas = self._create_user_personas()
        
        # **THE FIX: Dynamically create the persona list from the dictionary keys**
        ...
        # **NEW: Store the full feature dimension for agents to use**
        self.feature_dim = len(self.persona_names) + len(PRODUCT_COLS)
...
# --- Helper Functions ---
def create_feature_vectors(user_persona: str, products_df: pd.DataFrame, all_persona_names: List[str]) -> torch.Tensor:
    """
    Creates one-hot encoded feature vectors for all products for a given user.
    This version takes the list of all possible personas to ensure correctness.
    """
    # Use pandas.Categorical to handle one-hot encoding robustly.
    # This ensures that even if a persona is not in the list, it won't error out,
    # and the columns will always be in the specified order.
    user_cat = pd.Categorical([user_persona], categories=all_persona_names)
    persona_features = pd.get_dummies(user_cat, dtype=float) # Use float for compatibility
    
    product_cat = pd.Categorical(products_df['category'], categories=PRODUCT_COLS)
    product_features = pd.get_dummies(product_cat, dtype=float)
...
def run_simulation(agent: Any, simulator: ZooplusSimulator, num_interactions: int) -> List[int]:
    ...
    for _ in tqdm(range(num_interactions)):
        ...
        # **THE FIX: Pass the dynamic list of personas from the simulator**
        all_feature_vectors = create_feature_vectors(user_persona, simulator.products, simulator.persona_names)
